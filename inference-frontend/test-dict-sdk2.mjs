...
async function main() {
  const blockRes = await client.getBlock();
  const stateRootHash = blockRes.block?.header?.stateRootHash;
  console.log('stateRootHash:', stateRootHash?.toHex?.() || stateRootHash);
  ...
  const req = new sdk.StateGetDictionaryRequest(stateRootHash, dictId);
  const res = await client.getDictionaryItem(req);
