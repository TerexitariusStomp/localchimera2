...
CONFIG_SRC="$REPO_ROOT/inference-config/relayer.local.testnet.yaml"
CONFIG_DST="$RELAYER_DIR/config/local.testnet.yaml"
...
if [ ! -f "$ENV_DST" ]; then
  ...
  cp "$ENV_SRC" "$ENV_DST"
  echo "Required: set APP_GATEWAY__TX_ENGINE__SIGNER__PRIVATE_KEY with your funded Gateway testnet key."
  exit 1
fi
...
CI=1 YES=1 make preflight-testnet
