...
const WASM_PATH = '/home/user/CascadeProjects/chimera-fortytwo-node/contracts-casper/target/wasm32v1-none/release/escrow_vault.wasm';
...
const reputation = sdk.CLValue.newCLByteArray(
  Uint8Array.from(Buffer.from('fd0bf02161433c13c3070b7d0ea383c976bcbc799413638b4fedc703d4efa1db', 'hex'))
);
...
const nk = keys.find((k: any) => k.name === 'escrow_vault_hash');
if (nk) {
  const hash = nk.key.replace('hash-', '');
  console.log('\nNew EscrowVault hash:', hash);
  console.log('\nUpdate casper-client.ts:');
  console.log(`  escrowVault: '${hash}',`);
}
