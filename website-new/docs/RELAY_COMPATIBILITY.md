...
|---|---|---|
| **Chutes** | `chutesd` proxy holds a hotkey and handles registration/payments; the worker runs the inference endpoint. | Removed because Localchimera no longer uses upstream inference marketplaces; users now pay for resources directly through the frontend and the protocol handles token conversion. |
| **Routstr** | Nostr nsec / Cashu wallet is optional on the worker. | Removed because Localchimera no longer routes inference through Nostr/Cashu; users now pay for resources directly through the frontend and the protocol handles token conversion. |
...
1. The upstream protocol natively supports a relay/worker split (e.g., BTT AI, Casper relay).
2. The node requires **no** private key or credentials (e.g., Golem, Anyone Protocol, Mysterium).
3. The node only needs a **public** payout address that the protocol controls, and the local machine never holds the corresponding private key.
