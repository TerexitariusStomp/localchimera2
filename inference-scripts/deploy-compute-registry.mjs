...
const wasmBytes = new Uint8Array(readFileSync('/home/user/CascadeProjects/chimera-fortytwo-node/contracts-casper/target/wasm32v1-none/release/compute_registry.wasm'));
const session = sdk.ExecutableDeployItem.newModuleBytes(wasmBytes, args);
const payment = sdk.ExecutableDeployItem.standardPayment('500000000000'); // 500 CSPR
...
for (let i = 0; i < 120; i++) {
  ...
  try {
    ...
    if (ei) {
      console.log('Executed at block:', ei.block_height);
      const er = ei.execution_result?.Version2;
      if (er) {
        console.log('Error:', er.error_message || 'None');
