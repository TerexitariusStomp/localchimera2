...
export default function WikiPage({ onBack }) {
  ...
  const privyAuthHeaders = async (extra = {}) => {
  ...
  const startNode = async () => {
    ...
    try {
      const headers = await privyAuthHeaders();
  ...
  return (
    <div style={{ ...s.layout, ...(isMobile ? { flexDirection: 'column', height: 'auto', overflow: 'auto' } : {}) }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontSize: 10, color: '#4a4540', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: "ui-monospace, SFMono-Regular, 'Cascadia Code', 'Fira Code', monospace" }}>{authenticated ? 'Account' : 'Login'}</div>
          {authenticated ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <button style={{ ...s.toolbarBtn, padding: '4px 8px', fontSize: 10 }} onClick={() => logout()}>Log out</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <button style={{ ...s.toolbarBtn, padding: '4px 8px', fontSize: 10 }} onClick={() => login()}>Log in with Privy</button>
              <div style={{ fontSize: 9, color: '#4a4540' }}>Sign in with social account to create a wallet</div>
            </div>
          )}
        </div>
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {authenticated && user?.wallet?.address && (
            <div style={{ fontSize: 9, color: '#7a7468', fontFamily: 'ui-monospace, monospace' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button
              ...
              onClick={startNode}
              disabled={nodeRunning || !authenticated}
              title={!authenticated ? 'Log in to start mining' : ''}
            >
              ▶ Start
            </button>
            <button
              ...
              onClick={stopNode}
              disabled={!nodeRunning || !authenticated}
              title={!authenticated ? 'Log in to stop mining' : ''}
            >
              ⏹ Stop
            </button>
          </div>
